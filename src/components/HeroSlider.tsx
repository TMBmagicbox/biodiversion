...
export type HeroSlide = {
  ...
  imagen_decorativa_url: string | null;
...
export default function HeroSlider({ slides }: { slides: HeroSlide[] }) {
  ...
  // Parallax: el fondo y la imagen decorativa se mueven a distinta
  // velocidad que el scroll de la página, para dar sensación de profundidad.
  useEffect(() => {
    function onScroll() {
    ...
    >
      {data.map((slide, i) => (
        ...
        >
          ...
          {/* Imagen decorativa (PNG, aparte del fondo): se mueve más rápido
           * que el fondo para que el banner se sienta dinámico/con capas. */}
          {slide.imagen_decorativa_url && (
            <div
              className="pointer-events-none absolute bottom-0 right-0 hidden w-[260px] will-change-transform sm:block sm:w-[340px] lg:w-[420px]"
              style={{ transform: `translateY(${offset * 1.8}px)` }}
            >
              <Image
